...
static int handle_single_philosopher(t_philo_data *data, int fork)
{
...
static int take_first_fork(t_philo_data *data, int fork)
{
    if (pthread_mutex_lock(&(data->params->forks[fork])) != 0)
        return (1);
...
static int take_second_fork(t_philo_data *data, int first, int second)
{
...
int take_forks(t_philo_data *data, int left_fork, int right_fork)
{
    ...
    first_fork = (data->id % 2 == 0) ? right_fork : left_fork;
    second_fork = (data->id % 2 == 0) ? left_fork : right_fork;
